...
2. **分析剧本结构**：先在思考中规划镜头划分（不调用工具），确定镜头数量和每个镜头的核心内容
3. **生成分镜数据**：为所有镜头生成完整的分镜数据
4. **保存分镜**：调用save_storyboards保存所有分镜。优先尝试一次性保存所有分镜。如果保存失败（如输出被截断、JSON解析错误），请立即分批保存——每次调用save_storyboards只传3-5个分镜，分多次调用直到所有分镜都保存成功
5. **确认完成**：报告保存结果
